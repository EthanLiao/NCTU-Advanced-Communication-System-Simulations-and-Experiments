...
t_DMA_filtered_signal = filter(zero,pole,DAC(t_Digital_filtered_signal,DMA_sampling_factor))
...
r_DMA_filtered_signal = ADC(filter(zero,pole,demodulation_signal),DMA_sampling_factor)
...
subplot(4,2,7);stem(real(r_Digital_filtered_signal)./max(abs(real(r_Digital_filtered_signal))));hold on;stem(shift(signal,1));title('comparison two signal')


function shift_signal = shift(signal,shift)
new_size = length(signal)+1
shift_signal = zeros(1,new_size)
shift_signal(1+shift:end)=signal(1:end)
end
