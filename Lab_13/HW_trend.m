...
function  [zf_snr, mmse_snr]= MIMO_eval(SNR_DB, sig)
  % transmit sig
  t_sig_1 = trans_branch(sig(1,:));
  t_sig_2 = trans_branch(sig(2,:));
  ...
  % recieve signal
  mimo_rcv_1 = recieve_branch(add_awgn_noise(mimo_rcv(1,:), SNR_DB));
  mimo_rcv_2 = recieve_branch(add_awgn_noise(mimo_rcv(2,:), SNR_DB));
  ...
  mmse_rcv_beam = mmse_rcv_beam / max(mmse_rcv_beam,[] ,'all');
